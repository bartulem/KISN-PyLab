    ...
    def splitClusters(self, **kwargs):
        ...
        **kwargs: dictionary
        ...
        if (not onesession):
            ...
            omniPresent = 0
            allSessions = ['session_{}'.format(x + 1) for x in range(len(fileLengths.keys()) - 1)]
            for acell in allSpikeData.keys():
                if(list(allSpikeData[acell].keys()) == allSessions):
                    omniPresent += 1

            print('{} cells were present in all sessions'.format(omniPresent))
